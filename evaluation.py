...
def compute_oscr(x1, x2, correct):
...
def metrics(x1, x2, correct_flag, stypes=['Bas'], verbose=True):
    tp, fp, tnr_at_tpr95 = get_curve_online(x1, x2, stypes)
    results = dict()
    mtypes = ['TNR', 'AUROC', 'DTACC', 'AUIN', 'AUOUT', 'ACC', 'OSCR']
    if verbose:
        print('      ', end='')
        for mtype in mtypes:
    ...
    for stype in stypes:
        ...
        # ACC
        mtype = 'ACC'
        results[stype][mtype] = 100. * correct_flag.sum() / len(correct_flag)
        if verbose:
            print(' {val:6.3f}'.format(val=results[stype][mtype]), end='')

        # OSCR
        mtype = 'OSCR'
        results[stype][mtype] = 100. * compute_oscr(x1, x2, correct_flag)
        if verbose:
            print(' {val:6.3f}'.format(val=results[stype][mtype]), end='')
            print('')
